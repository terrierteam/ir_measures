...
def compute_metrics(qids_to_relevant_passageids, qids_to_ranked_candidate_passages, max_rank=MaxMRRRank):
    """Compute MRR metric
    Args:    
    p_qids_to_relevant_passageids (dict): dictionary of query-passage mapping
        Dict as read in with load_reference or load_reference_from_stream
    ...
    for qid in qids_to_ranked_candidate_passages:
        if qid in qids_to_relevant_passageids:
            ...
            for i, pid in enumerate(candidate_pid[:max_rank]):
                if pid in target_pid:
                    MRR += 1/(i + 1)
                    mrr_by_query[qid] = 1/(i + 1)
                    ranking.pop()
                    ranking.append(i+1)
                    break
    # if len(ranking) == 0:
...
def compute_metrics_from_files(path_to_reference, path_to_candidate, perform_checks=True, max_rank=MaxMRRRank):
    """Compute MRR metric
    Args:    
    ...
    return compute_metrics(qids_to_relevant_passageids, qids_to_ranked_candidate_passages, max_rank)
